//  Copyright (c) 2014 Scott Talbot. All rights reserved.

#import "STAppleRootCertificate.h"


static uint8_t const STAppleIncRootCertificateBytes[1215] = {
    
};
static size_t const STAppleIncRootCertificateLength = sizeof(STAppleIncRootCertificateBytes);

static uint8_t const STAppleComputerIncRootCertificateBytes[1470] = {
    
};
static size_t const STAppleComputerIncRootCertificateLength = sizeof(STAppleComputerIncRootCertificateBytes);


NSData *STAppleIncRootCertificateData(void) {
    return [[NSData alloc] initWithBytesNoCopy:(void *)STAppleIncRootCertificateBytes length:STAppleIncRootCertificateLength freeWhenDone:NO];
}
NSData *STAppleComputerIncRootCertificateData(void) {
    return [[NSData alloc] initWithBytesNoCopy:(void *)STAppleComputerIncRootCertificateBytes length:STAppleComputerIncRootCertificateLength freeWhenDone:NO];
}
